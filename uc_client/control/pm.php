...
class pmcontrol extends base {
	...
	function onignore() {
 	...
 	function onls() {
		...
		$dateline = intval($this->input('dateline'));
		$queryop = $this->input('queryop');
		...
		if($folder != 'searchbox') {
 		...
 		if($pagesize > 0) {
	 		$pms = $_ENV['pm']->get_pm_list($this->user['uid'], $pmnum, $folder, $filter, $start, $pagesize, $dateline, $queryop);
