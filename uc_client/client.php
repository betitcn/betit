 ...
 */
function uc_pm_list($uid, $page = 1, $pagesize = 10, $folder = 'inbox', $filter = 'newpm', $msglen = 0, $dateline=0, $queryop='up') {
	$uid = intval($uid);
	$page = intval($page);
	$pagesize = intval($pagesize);
	$return = call_user_func(UC_API_FUNC, 'pm', 'ls', array('uid'=>$uid, 'page'=>$page, 'pagesize'=>$pagesize, 'folder'=>$folder, 'filter'=>$filter, 'msglen'=>$msglen, 'dateline'=>$dateline, 'queryop'=>$queryop));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
 ...
 * @return array() 短消息内容数组
 */
function uc_pm_view2($uid, $pmid, $touid = 0, $daterange = 1, $page=1, $pagesize=10) {
	$uid = intval($uid);
	$touid = intval($touid);
	$pmid = @is_numeric($pmid) ? $pmid : 0;
	$return = call_user_func(UC_API_FUNC, 'pm', 'view', array('uid'=>$uid, 'pmid'=>$pmid, 'touid'=>$touid, 'daterange'=>$daterange, 'page'=>'page', 'pagesize'=>'pagesize'));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
