...
if($count) {
	...
	$quizids =  $posts = $comments = $ids = $blogids = $picids = $albumids = $spaceids = $pollids = $mtagids = $threadids = $shareids = $eventids = $shareids = array();
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		switch($value['idtype']) {
			case 'blogid':
				...
				break;
			case 'quizid':
				$quizids[$value['id']] = $value['id'];
				$list['quiz'][$value['id']] = $value;
				break;
			case 'picid':
	...
	if($quizids) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			$list['quiz'][$value['quizid']]['info'] = $value;
			unset($emptyids['quizid'.$value['quizid']]);
		}
	}
	//图片
	if($picids) {
	...
	if($comments) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			switch ($value['idtype']) {
				...
				case 'quizid':
					//竞猜
					$url .= "quiz&id=$value[id]&cid=$value[cid]";
					break;
				case 'sid':
					//分享
	...
	foreach($deltype as $key => $value) {
		switch($key) {
			case 'blogid':
				...
				break;
			case 'quizid':
				$_SGLOBAL['usergroup'][$gid]['managequiz'] = 1;
				deletequizs($value);
				break;
			case 'picid':
