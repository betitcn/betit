...
/*
	...
	$Id: function_common.php 13519 2010-01-0
5 02:42:46Z liguode $
*/
...
function checkauth() {
	...
	if($_SGLOBAL['mobile'] && $_REQUEST['m_auth']) $_SCOOKIE['auth'] = $_REQUEST['m_auth'];
	if($_SCOOKIE['auth']) {
			...
			if($member = $_SGLOBAL['db']->fetch_array($query)) {
				...
				if($member = $_SGLOBAL['db']->fetch_array($query)) {
					if($member['password'] == $password) {
						...
						$session = insertsession($session);//登录
					} else {
						$_SGLOBAL['supe_uid'] = 0;
					}
				} else {
...
function apple_push($token,$msg, $userinfo='', $debug=0){
	...
	include_once S_ROOT.'./source/APN/APFeedback.php';
	...
	try{
	  ...
	  if ($userinfo!=='')
		 $notification->addProperty("userinfo", $userinfo);
	  $notification->setPrivateKey( S_ROOT."./ctf/apns_dev.pem" );
	...
	require_once S_ROOT.'./source/ApnsPHP/Autoload.php';
	...
	if ( $debug )
		$push = new ApnsPHP_Push(
			...
			S_ROOT."./ctf/apns_dev.pem" 
		);
	else
		$push = new ApnsPHP_Push(
			ApnsPHP_Abstract::ENVIRONMENT_PRODUCTION,
			S_ROOT."./ctf/apns_pro.pem" 
		);

	// Set the Root Certificate Autority to verify the Apple remote peer
	//$push->setRootCertificationAuthority( S_ROOT."./ctf/apns_dev.pem" );
	...
	$message->setCustomIdentifier($msg);
	...
	// Set a custom property
	 if ($userinfo!=='')
		$message->setCustomProperty('userinfo', $userinfo);
