...
if($op == 'comment') {
	...
	$idtype = (empty($_GET['idtype']) || !in_array($_GET['idtype'], $idtypes))?'quizid':$_GET['idtype'];
	if($page<1) $page=1;
	//评论
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('comment')." WHERE $csql id='$id' AND idtype='$idtype' ORDER BY dateline LIMIT $start,$perpage");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		realname_set($value['authorid'], $value['author']);//实名
		$list[] = $value;
	}
	
	$multi = smulti($start, $perpage, $count, "do.php?ac=ajax&op=getcomment&id=$id&idtype=$idtype", $_GET['ajaxdiv']);
	realname_get();
