...
function feed_publish($id, $idtype, $add=0) {
	...
	switch ($idtype) {
		case 'blogid':
			...
			break;
		case 'quizid':
			$query = $_SGLOBAL['db']->query("SELECT b.*, bf.* FROM ".tname('quiz')." b
				LEFT JOIN ".tname('quizfield')." bf ON bf.quizid=b.quizid
				WHERE b.quizid='$id'");
			$optionstr = '';
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
			
				$query2 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('quizoptions')."  WHERE quizid='$id'");
				$value["options"]=array();
				while($value2 = $_SGLOBAL['db']->fetch_array($query2)){
					$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('quizuser')." WHERE oid='$value2[oid]' "), 0);
					$count = empty($count)?0:$count;
					$optionstr[]= ''.''.$count.'';
					if ($value2['picid']){
						$query3 = $_SGLOBAL['db']->query("SELECT * FROM ".tname('pic')." WHERE picid='$value2[picid]'");
						$value3 = $_SGLOBAL['db']->fetch_array($query3);
						$value2['url'] = pic_get($value3['filepath'], $value3['thumb'], $value3['remote']);
						$value2['pic'] = $value3['filepath'];
					}
				...
				if($value['friend'] != 3) {
					...
					//详细
					$url = "space.php?uid=$value[uid]&do=quiz&id=$value[quizid]";
					if($value['friend'] == 4) {//加密
						$setarr['title_template'] = cplang('feed_quiz_password');
						$setarr['title_data'] = array('subject' => "<a href=\"$url\">$value[subject]</a>");
					} else {//非私人
	...
	if($setarr['icon']) {
		...
		$setarr['hash_template'] = md5($setarr['title_template']."\t".$setarr['message_template']);//喜好hash
		$setarr['hash_data'] = md5($setarr['title_template']."\t".$setarr['title_data']."\t".$setarr['message_template']."\t".$setarr['body_data']);//合并hash
		...
		if($feedid) {
			...
			$feedid = inserttable('feed', $setarr);
		}

		/* $query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('feed')." WHERE feedid='$feedid' ");
		if($pushfeed = $_SGLOBAL['db']->fetch_array($query)) {
			include_once(S_ROOT.'./capi/function_capi.php');
			$pushfeed = capi_mkfeedtitle($pushfeed);
			apple_push("00587944741d507610cb74a6545a34486184497ffba965ae4b3c8aa3cc7b4790",$pushfeed["title_template"],$setarr);
		} */
	}
