...
function hot_update($idtype, $id, $hotuser) {
	...
	switch ($idtype) {
		case 'blogid':
			...
			break;
		case 'quizid':
			$_SGLOBAL['db']->query("UPDATE ".tname('quizfield')." SET hotuser='$hotuser' WHERE quizid='$id'");
			$_SGLOBAL['db']->query("UPDATE ".tname('quiz')." SET hot=hot+1 WHERE quizid='$id'");
			break;
		case 'tid':
