...
//竞猜
$cachefile = S_ROOT.'./data/cache_network_quiz.txt';
if(check_network_cache('quiz')) {
	$bloglist = unserialize(sreadfile($cachefile));
} else {
	$sqlarr = mk_network_sql('quiz',
		array('quizid', 'uid'),
	...
	$query = $_SGLOBAL['db']->query("SELECT main.*, field.* 
		FROM ".tname('quiz')." main
		LEFT JOIN ".tname('quizfield')." field ON field.quizid=main.quizid
	...
	if($_SGLOBAL['network']['quiz']['cache']) {
		swritefile($cachefile, serialize($quizlist));
	}
}
