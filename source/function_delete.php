...
function deletecomments($cids) {
	...
	$deductcredit = array();
	$blognums = $spaces = $newcids = $dels = $quiznums = array();
	...
	$nums = renum($quiznums);
	foreach ($nums[0] as $num) {
		$_SGLOBAL['db']->query("UPDATE ".tname('quiz')." SET replynum=replynum-$num WHERE quizid IN (".simplode($nums[1][$num]).")");
	}
	return $dels;
}
...
function deletequizs($quizids) {
	global $_SGLOBAL;

	//获取积分
	$reward = getreward('delquiz', 0);
	//获取博客信息
	$spaces = $quizs = $newquizids = array();
	$allowmanage = checkperm('managequiz');
	$managebatch = checkperm('managebatch');
	$delnum = 0;
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('quiz')." WHERE quizid IN (".simplode($quizids).")");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if($allowmanage || $value['uid'] == $_SGLOBAL['supe_uid']) {
			$quizs[] = $value;
			if(!$managebatch && $value['uid'] != $_SGLOBAL['supe_uid']) {
	...
	if(empty($quizs) || (!$managebatch && $delnum > 1)) return array();
	
	foreach($quizs as $key => $value) {
		$newquizids[] = $value['quizid'];
		if($allowmanage && $value['uid'] != $_SGLOBAL['supe_uid']) {
			//扣除积分
			$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET credit=credit-$reward[credit], experience=experience-$reward[experience] WHERE uid='$value[uid]'");
		}
		//tag
		$tags = array();
		$subquery = $_SGLOBAL['db']->query("SELECT tagid, quizid FROM ".tname('tagquiz')." WHERE quizid='$value[quizid]'");
		while ($tag = $_SGLOBAL['db']->fetch_array($subquery)) {
			$tags[] = $tag['tagid'];
		}
		if($tags) {
			$_SGLOBAL['db']->query("UPDATE ".tname('tag')." SET blognum=blognum-1 WHERE tagid IN (".simplode($tags).")");
			$_SGLOBAL['db']->query("DELETE FROM ".tname('tagquiz')." WHERE quizid='$value[quizid]'");
		}
	}

	//数据删除
	$_SGLOBAL['db']->query("DELETE FROM ".tname('quiz')." WHERE quizid IN (".simplode($newquizids).")");
	$_SGLOBAL['db']->query("DELETE FROM ".tname('quizfield')." WHERE quizid IN (".simplode($newquizids).")");

	//评论
	$_SGLOBAL['db']->query("DELETE FROM ".tname('comment')." WHERE id IN (".simplode($newquizids).") AND idtype='quizid'");

	//删除举报
	$_SGLOBAL['db']->query("DELETE FROM ".tname('report')." WHERE id IN (".simplode($newquizids).") AND idtype='quizid'");

	//删除feed
	$_SGLOBAL['db']->query("DELETE FROM ".tname('feed')." WHERE id IN (".simplode($newquizids).") AND idtype='quizid'");
	
	//删除脚印
	$_SGLOBAL['db']->query("DELETE FROM ".tname('clickuser')." WHERE id IN (".simplode($newquizids).") AND idtype='quizid'");
...
function deletespace($uid, $force=0) {
	...
	//quiz
	$quizids = array();
	$query = $_SGLOBAL['db']->query("SELECT quizid FROM ".tname('quiz')." WHERE uid='$uid'");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		$quizids[$value['quizid']] = $value['quizid'];
		//tag
		$tags = array();
		$subquery = $_SGLOBAL['db']->query("SELECT tagid, quizid FROM ".tname('tagquiz')." WHERE quizid='$value[quizid]'");
		while ($tag = $_SGLOBAL['db']->fetch_array($subquery)) {
	...
	//数据删除
	$_SGLOBAL['db']->query("DELETE FROM ".tname('quiz')." WHERE uid='$uid'");
	$_SGLOBAL['db']->query("DELETE FROM ".tname('quizfield')." WHERE uid='$uid'");
