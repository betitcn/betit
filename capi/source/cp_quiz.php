...
if(empty($quiz)) {
	if(!checkperm('allowquiz')) {
		ckspacelog();
		capi_showmessage_by_data('no_authority_to_add_bet');
	}
	...
	//接收外部标题
	$quiz['subject'] = empty($_REQUEST['subject'])?'':getstr($_REQUEST['subject'], 80, 1, 0);
	$quiz['message'] = empty($_REQUEST['message'])?'':getstr($_REQUEST['message'], 5000, 1, 0);
...
//添加编辑操作
if(capi_submitcheck('quizsubmit')) {
	...
	if($_SGLOBAL['timestamp']>$quiz['endtime']){
		capi_showmessage_by_data("亲！你知道吗！该竞猜已过期，不允许编辑或转发！");
		}
	}
	if($newquiz = quiz_post($_REQUEST)) {
		...
		$newquiz["reward"] = $reward;
		capi_showmessage_by_data('do_success', 0, array('quiz'=>$newquiz));
			}else{
        updatetable('quiz', array('fquizid'=>$fquizid),array('quizid'=>$newquiz['quizid']));
		$reward = getreward('publishquiz', 0); 
		$newquiz["reward"] = $reward;		$newquiz["fquizid"] = $fquizid;
		capi_showmessage_by_data('do_success', 0, array('quiz'=>$newquiz));
			}
	...
	if($newquiz = quiz_post($_REQUEST, $quiz)) {
		if(empty($quiz) && $newquiz['topicid']) {
			$url = 'space.php?do=topic&topicid='.$newquiz['topicid'].'&view=quiz';
		} else {
			$url = 'space.php?uid='.$newquiz['uid'].'&do=quiz&id='.$newquiz['quizid'];
...
if($_REQUEST['op'] == 'delete') {
	...
	capi_showmessage_by_data('do_success',0, "space.php?uid=$uid&do=quiz&id=$id");
	...
	if(capi_submitcheck('hotsubmit')) {
		...
		updatetable('quiz', array('hot'=>$_REQUEST['hot']), array('quizid'=>$quiz['quizid']));
		
		if($_REQUEST['hot']>0) {
			include_once(S_ROOT.'./source/function_feed.php');
			feed_publish($quiz['quizid'], 'quizid');
		} else {
			updatetable('feed', array('hot'=>$_REQUEST['hot']), array('id'=>$quiz['quizid'], 'idtype'=>'quizid'));
		}
		
		capi_showmessage_by_data('do_success', "space.php?uid=$quiz[uid]&do=quiz&id=$quizid", 0);
	}
	
} elseif($op == 'vote') {
	
	//计票
	if(capi_submitcheck('votesubmit')) {
		if(empty($quiz)) {
			capi_showmessage_by_data("voting_does_not_exist");
		}
		if($space['credit']<=$quiz['joincost']){
			capi_showmessage_by_data("voting_not_enough_credit");
		}
		//验证性别
		...
		if(!isset($_REQUEST['anonymous']) && $_SGLOBAL['supe_uid']!=$quiz['uid'] && ckprivacy('joinquiz', 1)) {
			$fs = array();
			$fs['icon'] = 'quiz';
		...
		$quiz["reward"] = $reward ;
		capi_showmessage_by_data('do_success', 0, array('quiz'=>$quiz));
	}
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('quiz')." WHERE quizid='$quizid'");
			$fkey = $_SGLOBAL['db']->fetch_array($query);
			$endtime=$fkey['endtime'];
			$resulttime=$fkey['resulttime']+86400;
			if($_SGLOBAL['timestamp']<$endtime||$_SGLOBAL['timestamp']>$resulttime){
				capi_showmessage_by_data("此时段不允许公布答案！");
				}
			$newfquizid=$fkey['fquizid'];
	...
	if(capi_submitcheck('keysubmit')) {
		
		$keyid = empty($_REQUEST['keyid'])?0:intval($_REQUEST['keyid']);
		...
		capi_showmessage_by_data('do_success', "space.php?uid=$quiz[uid]&do=quiz&id=$quizid");
				}
		...
		$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('quiz')." WHERE fquizid='$quizid' or quizid='$quizid' order by quizid DESC");
			...
			capi_showmessage_by_data('不好意思，此竞猜为转发，须原发布者公布答案！');
	}
	...
	if ($_REQUEST['filtrate']=='new' || $_REQUEST['filtrate']=='we'){
		...
		if($count) {
			...
			$multi = multi($count, $perpage, $page, "cp.php?ac=quiz&op=get&quizid=$quizid&filtrate=".$_REQUEST['filtrate'], 'showvoter');
			//实名
			realname_get();
		}
	}else{
		$wherearr = $voteresult = array();
		$wherearr[] = "quizid='$quizid'";
		if($_REQUEST['filtrate'] == 'win') {
			$wherearr[] = "winflag='1'";
		}elseif($_REQUEST['filtrate'] == 'lost') {
			$wherearr[] = "winflag='0'";
		}
		$wheresql = ' WHERE '.implode(' AND ', $wherearr);
	...
	if(capi_submitcheck('invitesubmit')) {
		...
		capi_showmessage_by_data('do_success', 0,'space.php?uid='.$quiz['uid'].'&do=quiz&id='.$quizid);
	}
	...
	$multi = multi($count, $perpage, $page, "cp.php?ac=quiz&op=invite&quizid=$quiz[quizid]&group=$_REQUEST[group]&key=$_REQUEST[key]");
